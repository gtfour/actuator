...
func Shifter(interval [][]int)(ninterval [][]int) {
    ...
    for i:= range interval {
        if analyze.IsDigitIn(i,skipped) == false {
            ...
            for z:= range interval {
                ...
                if analyze.DigitInInterval(first,parent_int_part) == DIGIT_IN_INTERVAL && analyze.DigitInInterval(last,parent_int_part) == DIGIT_IN_INTERVAL {
    ...
    for x:= range interval {
        if analyze.IsDigitIn(x,skipped) == false {
...
func AlumaPaster(delims [][]int, data [][]int, strada [][]int)(ndelims [][]int, ndata [][]int){
    ...
    for i := range strada {
        ...
        for de := range  delims {
            ...
            first_delim       := delim[0]
            last_delim        := delim[1]
            first_state       := analyze.DigitInInterval(first, delim)
            last_state        := analyze.DigitInInterval(last, delim)
            first_delim_state := analyze.DigitInInterval(first_delim, indexes)
            last_delim_state  := analyze.DigitInInterval(last_delim,  indexes)
    ...
    for da := range  data {
        ...
        for i := range strada {
            ...
            first_state          := analyze.DigitInInterval(first_data, indexes)
            last_state           := analyze.DigitInInterval(last_data, indexes)
            if first_state == DIGIT_IN_INTERVAL && last_state == DIGIT_IN_INTERVAL{
                includes = true
                ...
                if  analyze.DigitInInterval(first_strada, interval_between_data)  == DIGIT_IN_INTERVAL && analyze.DigitInInterval(last_strada, interval_between_data)  == DIGIT_IN_INTERVAL {
                    last_matched_strada_id = i
